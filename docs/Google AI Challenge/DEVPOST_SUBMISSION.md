...
We needed an agent that was blazingly fast, deeply integrated into our Property Management System (PMS), and provably immune to hallucination under adversarial conditions.

---

## ⚙️ Architecture: Two-Path Design

To achieve sub-900ms audio response while keeping all AI intelligence on Google's infrastructure, we designed a Two-Path Architecture that separates the real-time speech I/O layer from the Gemini reasoning layer.
