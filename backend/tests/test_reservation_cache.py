...
async def test_the_same_name_in_different_case_is_one_query():
    """
    Live trace CAfdd9c18f846e3c20ca3db5dbc305cf53: the model asked for
    `guest_name="Drew Patel"` and later `guest_name="drew patel"`. The database
    layer title-cases the name before querying, so both produce an identical
    query — but the memo keyed on the raw argument and paid for it twice
    (1.72s, then 505ms).
    """
    db = FakeDB()
    cached = _CachedReservationLookup(db)

    await cached.lookup_motel_reservation(guest_name="Drew Patel", tenant_id="coalcreek")
    await cached.lookup_motel_reservation(guest_name="drew patel", tenant_id="coalcreek")
    await cached.lookup_motel_reservation(guest_name="  DREW PATEL  ", tenant_id="coalcreek")
...
async def test_reference_and_email_case_are_also_one_query():
    """The database upper-cases references and lower-cases emails. The key must agree."""
    db = FakeDB()
    cached = _CachedReservationLookup(db)

    await cached.lookup_motel_reservation(booking_reference="cc-76818", tenant_id="coalcreek")
    await cached.lookup_motel_reservation(booking_reference="CC-76818", tenant_id="coalcreek")
    await cached.lookup_motel_reservation(email="Guest@Example.com", tenant_id="coalcreek")
    await cached.lookup_motel_reservation(email="guest@example.com", tenant_id="coalcreek")

    assert len(db.calls) == 2, f"expected 2 round trips, made {len(db.calls)}"


async def test_the_database_receives_the_normalised_value():
    """Normalising for the key must not change what the query actually asks for."""
    db = FakeDB()
    cached = _CachedReservationLookup(db)

    await cached.lookup_motel_reservation(guest_name="  drew patel ", tenant_id="coalcreek")

    assert db.calls == [{"guest_name": "Drew Patel", "tenant_id": "coalcreek"}]
