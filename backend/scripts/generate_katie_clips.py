#!/usr/bin/env python3
"""
Generate Katie US system audio clips for Ovela AI.

Voice: Katie US  (f9836c6e-a0bd-460e-9d3c-f7299fa60f94)
...
KATIE_VOICE_ID = "f786b574-daa5-4673-aa0c-cbe3e8534c02"  # Katie US
OUTPUT_DIR = Path(__file__).resolve().parent / "audio" / KATIE_VOICE_ID
...
def synthesise(text: str) -> bytes:
    payload = {
        "model_id": "sonic-3.5",
        "transcript": text,
        "voice": {"mode": "id", "id": KATIE_VOICE_ID},
        "output_format": {
            "container": "raw",
            "encoding": "pcm_mulaw",
            "sample_rate": 8000,
...
def main() -> None:
    ...
    print(f"🎙️  Katie US voice: {KATIE_VOICE_ID}")
    ...
    if failures:
        ...
        print("🎉 All clips generated. Katie voice cache is complete.")
        print()
        print("Next step: remove the 'blake_fallback' block in handler.py")
        print("           _get_clip_path() — Katie folder now exists.")
